...
`node shipward/setup.mjs /path/to/repo [--name N] [--prefix PX] [--seed-from-branches]` wires any git repository to this install in one idempotent command: seeds the target's **own** `.shipward/tracker.json` (each repo keeps its own memory — trackers are never pooled), merges the four hooks + statusline into its `.claude/settings.json`, registers the MCP server in its `.mcp.json`, and appends the protocol to its `CLAUDE.md`.

**`--seed-from-branches` gives the repo a first board that is not empty** (SW-067). An empty board cannot be corrected by git, so the one thing worth seeing — the audit fixing the board unasked — could not fire until the user had logged, worked and landed a card, days later. It files one `backlog` card per branch that is **unmerged and ahead of the trunk**: title from the branch name, type from its `feat/fix/chore` prefix, `branch` set, and a `brief` note carrying the commit subjects. Merged branches are finished work and are never filed; a ref with nothing ahead is not work. **`commit` is left null deliberately** — a branch head baked in at install time is stale on the next commit, and *names a branch but records no sha* is a `certain` finding, so the first SessionStart fills it in and writes the `[git audit]` line. Seeding is capped at 25 and **reports what the cap dropped**; a branch some card already names is skipped, so re-running is safe. Every run, flag or not, ends with a read-only preview of what git holds and the exact command to seed — an opt-in nobody sees is the empty board with extra steps. The central tools resolve by where they run — env `SHIPWARD_TRACKER`/`SHIPWARD_REPO` first, then the repo you are standing in, then this one — so `node <here>/shipward/serve.mjs` from inside an onboarded repo serves *that* repo's board. `node shipward/fleet.mjs ~/projects` (port 4740) shows **every** onboarded board on one page — with one standup across all of them at the top: what is in flight anywhere, what has been waiting on you longest, and which repos have gone quiet (nothing pushed in 21 days, kept distinct from *never* pushed). Boards the walk found but the fleet did not show are **reported**, never dropped silently: it runs at most 16 desks, and a cross-repo answer that omits a repo without saying so reads as full coverage. The rules are pure and live in `public/fleet-digest.js` — and `node shipward/fleet-service.mjs install ~/projects` makes it permanent (a macOS LaunchAgent: starts at login, restarts on crash, logs to ~/Library/Logs/shipward-fleet.log; `status`/`uninstall` to inspect or remove) — it scans two levels deep for trackers and spawns one ordinary desk per board, so clicking a name opens that repo's full desk.
